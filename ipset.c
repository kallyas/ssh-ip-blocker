 ...
 */
static bool try_ipset_cmd(enum ipset_cmd cmd, const char *setname,
...
static bool try_ipset_create(const char *setname, const char *typename) {
  const struct ipset_type *type;
  uint32_t timeout;
  uint8_t family;
  int r;
...
void blocker_fini(void) { ipset_session_fini(session); }