...
static bool find_ip(const pcre *code, const char *subject, int length,
                    struct in_addr *addr) {
  int rc, options = 0;
  ...
  rc = pcre_exec(code, NULL, subject, length, 0, options, ovector,
  ...
  if (pcre_copy_named_substring(code, subject, ovector, rc, "ip", ip,
                                sizeof ip) < 0) {
...
static void process_line(struct log_pattern *patterns, int patterns_count,
...
static int drop_privileges(uid_t uid, gid_t gid) {
  /* CAP_NET_ADMIN: necessary for ipset; CAP_SETUID, CAP_SETGID:
   * setresgid/setresguid */
